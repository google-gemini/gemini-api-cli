...
var listCmdMeta = []flagutil.FlagMeta{
	{FlagName: "filter", Shorthand: "f", FieldPath: "Filter", Kind: flagutil.FlagKindString, Optional: true, Description: "Optional. Filter expression (e.g., by state)."},
	{FlagName: "page-size", FieldPath: "PageSize", Kind: flagutil.FlagKindInt64, Optional: true, Description: "Optional. The maximum number of triggers to return per page."},
	{FlagName: "page-token", FieldPath: "PageToken", Kind: flagutil.FlagKindString, Optional: true, Description: "Optional. A page token from a previous ListTriggers call."},
}
...
func initListCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		Use:     "list",
		Short:   "List triggers for a project",
		Long:    "Lists triggers for a project.",
		Example: "  gemini-api triggers list",
		Args:    cobra.NoArgs,
		RunE:    runListCmd,
		Annotations: map[string]string{
			"speakeasy_operation": "ListTriggers",
		},
	}
	flagutil.RegisterFlags(cmd, listCmdMeta)
	if err := flagutil.ValidateMeta[operations.ListTriggersRequest](listCmdMeta); err != nil {
...
func runListCmd(cmd *cobra.Command, args []string) error {
	...
	req, err := flagutil.BuildRequest[operations.ListTriggersRequest](cmd, listCmdMeta, "", "")
	if err != nil {
	...
	if allPages && !client.IsDryRun(cmd) {
		res, err := s.Triggers.List(cmd.Context(), req, sdkOpts...)
		if err != nil {
			return output.Error(cmd, err)
		}
		return output.PaginatedResult(cmd, res, "ListTriggersResponse", "", maxPages, output.PaginationProbe{
			Type:       "cursor",
			CursorKind: "string",
