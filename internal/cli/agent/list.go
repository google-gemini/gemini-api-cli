...
var listCmdMeta = []flagutil.FlagMeta{
	...
	{FlagName: "parent", FieldPath: "Parent", Kind: flagutil.FlagKindString, Optional: true, Description: "string value"},
}
...
func initListCmd(parent *cobra.Command) error {
	var cmd = &cobra.Command{
		Use:     "list",
		Short:   "List managed agent definitions",
		Long:    "Lists all Agents.",
		Example: "  gemini-api agent list",
		Args:    cobra.NoArgs,
		RunE:    runListCmd,
		Annotations: map[string]string{
			"speakeasy_operation": "ListAgents",
		},
	}
	flagutil.RegisterFlags(cmd, listCmdMeta)
	if err := flagutil.ValidateMeta[operations.ListAgentsRequest](listCmdMeta); err != nil {
...
func runListCmd(cmd *cobra.Command, args []string) error {
	...
	req, err := flagutil.BuildRequest[operations.ListAgentsRequest](cmd, listCmdMeta, "", "")
	if err != nil {
	...
	if allPages && !client.IsDryRun(cmd) {
		res, err := s.Agent.List(cmd.Context(), req, sdkOpts...)
		if err != nil {
			return output.Error(cmd, err)
		}
		return output.PaginatedResult(cmd, res, "AgentListResponse", "", maxPages, output.PaginationProbe{
			Type:       "cursor",
			CursorKind: "string",
